...
const PrivacyPolicy = ({
  ...
  useEffect(async () => {
    await dispatch(fetchPrivacy()).then(unwrapResult).then(res => {
  ...
  return <View style={_styles.xWxtyrmL}>
      ...
      <ScrollView style={styles.privacyContainer}>
        <HTML source={{
        html: htmlContent
      }} contentWidth={contentWidth} style={[styles.heading, contentContainerStyle]} />
      </ScrollView>
    </View>;
};
